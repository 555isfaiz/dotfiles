...
window_state() {
  #source "$HOME/.config/sketchybar/colors_dark.sh"
  ...
  #args=()
    ...
    #case "$(echo "$WINDOW" | jq '.["is-floating"]')" in
      #"false")
          ...
          #args+=(--set $NAME icon=$YABAI_FULLSCREEN_ZOOM icon.color=$ORANGE)
        ...
        #else
          #args+=(--set $NAME icon=$YABAI_GRID icon.color=$TEAL)
          #yabai -m config active_window_border_color $TEAL > /dev/null 2>&1 &
        #fi
        #;;
      #"true")
