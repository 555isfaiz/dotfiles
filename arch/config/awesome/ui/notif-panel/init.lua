...
local nconstants = require("naughty.constants")
...
local notifs_container = wibox.widget {
  ...
  forced_width = 320,
  forced_height = 1800, --430, --Use it like in notifs_empty else it will look weird
...
local action = wibox {
  ...
  ontop = true,
  width = dpi(410),
  height = awful.screen.focused().geometry.height - dpi(80),
  y = dpi(60),
  bg = beautiful.bg_normal,
  border_width = dpi(3),
...
action.toggle = function()
  if action.visible then
    ...
    slide.target = awful.screen.focused().geometry.x - awful.screen.focused().geometry.width + action.width + dpi(18) --dpi(1465)
    action.visible = not action.visible
  end
end
...
naughty.connect_signal("destroyed", function(n, reason)
  ...
  if reason == nconstants.notification_closed_reason.dismissed_by_user then
    local jumped = false
...
-- helpers.click2hide.popup(action, function ()
--   action.toggle()
-- end, true)
