...
class PlayerManager:
    def __init__(self, selected_player=None, ignore_player=None):
    ...
    def init_players(self):
        for player in self.manager.props.player_names:
            if self.ignore_player is not None and self.ignore_player == player.name:
                continue
    ...
    def on_player_appeared(self, _, player):
        ...
        if (
            player is not None
            and (self.selected_player is None or player.name == self.selected_player)
            and (self.ignore_player is not None and self.ignore_player != player.name)
        ):
            self.init_player(player)
        else:
            logger.debug(
...
def parse_arguments():
    ...
    parser.add_argument("--ignore-player")
...
def main():
    ...
    player = PlayerManager(arguments.player, arguments.ignore_player)
    player.run()
